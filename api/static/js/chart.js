...
async function initMainChart(stockData) {
  ...
  const support = stockData.levels["1_mo"].support;
  const resistance = stockData.levels["1_mo"].resistance;
...
async function initRSIChart(stockData) {
  ...
  rsiSeries.setData(
    stockData.indicators.rsi.map((x) => ({
      time: x.time,
      value: x.rsi,
    }))
  );
...
async function main() {
  ...
  const stockData = await fetchData("/api/stock/" + urlParams.get("ticker"));
